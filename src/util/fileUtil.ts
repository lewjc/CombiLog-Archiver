...
export function writeToLogFile(directory: string, fileName: string, content: string) {
	const filePath = path.join(directory, fileName);
		...
		files.forEach((filePath: string) => {
			...
			paths.forEach((pathItem: string, idx: number) => {
				...
				switch (idx) {
					...
					case 1: {
						if (ref.children.filter((x: any) => x.name === pathItem).length === 0) {
							ref.children.push({
								id: uuid(),
								name: pathItem,
								children: [],
					...
					case 2: {
						const parent = ref.children.filter((x: any) => x.name === paths[idx - 1]).pop();
						if (parent.children.filter((x: any) => x.name === pathItem).length === 0) {
							const treeItem: any = {
								id: uuid(),
								name: pathItem,
							};
